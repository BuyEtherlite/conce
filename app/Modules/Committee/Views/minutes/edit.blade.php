    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4>Edit Meeting Minutes</h4>
                <a href="{{ route('committee.minutes.show', $minute->id) }}" class="btn btn-secondary">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('committee.minutes.update', $minute->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('meeting_id') is-invalid @enderror" id="meeting_id" name="meeting_id" required>
                                        ...
                                        @foreach($meetings as $meeting)
                                            <option value="{{ $meeting->id }}" {{ old('meeting_id', $minute->meeting_id) == $meeting->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('minute_type') is-invalid @enderror" id="minute_type" name="minute_type" required>
                                        ...
                                        <option value="discussion" {{ old('minute_type', $minute->minute_type) === 'discussion' ? 'selected' : '' }}>Discussion</option>
                                        <option value="resolution" {{ old('minute_type', $minute->minute_type) === 'resolution' ? 'selected' : '' }}>Resolution</option>
                                        <option value="decision" {{ old('minute_type', $minute->minute_type) === 'decision' ? 'selected' : '' }}>Decision</option>
                                        <option value="action_item" {{ old('minute_type', $minute->minute_type) === 'action_item' ? 'selected' : '' }}>Action Item</option>
                                        <option value="general" {{ old('minute_type', $minute->minute_type) === 'general' ? 'selected' : '' }}>General</option>
                                    </select>
                                    @error('minute_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" 
                                      id="content" name="content" rows="10" required>{{ old('content', $minute->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i>Update Minutes
                            </button>
                        </div>
                    </form>
                </div>
