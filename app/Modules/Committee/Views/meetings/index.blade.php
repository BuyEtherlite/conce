...
<nav class="navbar navbar-expand-lg navbar-light bg-light mb-4">
    <div class="container-fluid">
        ...
        <div class="navbar-nav">
            ...
            <a class="nav-link active" href="{{ route('committee.meetings.index') }}">
...
<div class="container-fluid">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Committee Meetings</h1>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $meeting->committee->name ?? 'N/A' }}</td>
                            <td>{{ $meeting->meeting_date->format('Y-m-d') }}</td>
                            <td>{{ $meeting->meeting_time ? $meeting->meeting_time->format('H:i') : 'N/A' }}</td>
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    <a href="{{ route('committee.meetings.show', $meeting) }}" class="btn btn-sm btn-outline-primary">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('committee.meetings.edit', $meeting) }}" class="btn btn-sm btn-outline-secondary">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form method="POST" action="{{ route('committee.meetings.destroy', $meeting) }}" class="d-inline">
                                        @csrf
                                        @method('DELETE')
        ...
        @else
            <div class="text-center py-5">
                <i class="fas fa-calendar-x display-1 text-muted"></i>
                <h5 class="mt-3">No Meetings Scheduled</h5>
                <p class="text-muted">There are no committee meetings scheduled yet.</p>
                <a href="{{ route('committee.meetings.create') }}" class="btn btn-primary">
