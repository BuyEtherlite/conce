...
<nav class="navbar navbar-expand-lg navbar-light bg-light mb-4">
    <div class="container-fluid">
        ...
        <div class="navbar-nav">
            ...
            <a class="nav-link active" href="{{ route('committee.resolutions.index') }}">
...
<div class="container-fluid">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Committee Resolutions</h1>
            <div class="btn-toolbar mb-2 mb-md-0">
                <a href="{{ route('committee.resolutions.create') }}" class="btn btn-primary">
            ...
            <table class="table table-striped table-sm">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $resolution->committee->name ?? 'N/A' }}</td>
                        <td>{{ $resolution->meeting->meeting_date ?? 'N/A' }}</td>
                        <td>{{ $resolution->resolution_date ? $resolution->resolution_date->format('Y-m-d') : 'N/A' }}</td>
                        <td>
                            <span class="badge badge-{{ $resolution->status === 'approved' ? 'success' : ($resolution->status === 'pending' ? 'warning' : 'danger') }}">
                                {{ ucfirst($resolution->status) }}
                            </span>
                        </td>
                        <td>{{ ucfirst($resolution->voting_result) }}</td>
                        <td>
                            <div class="btn-group" role="group">
                                <a href="{{ route('committee.resolutions.show', $resolution) }}" class="btn btn-sm btn-outline-primary">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('committee.resolutions.edit', $resolution) }}" class="btn btn-sm btn-outline-secondary">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="{{ route('committee.resolutions.download', $resolution) }}" class="btn btn-sm btn-outline-info">
                                    <i class="fas fa-download"></i>
                                </a>
                                <form method="POST" action="{{ route('committee.resolutions.destroy', $resolution) }}" class="d-inline">
                                    @csrf
                                    @method('DELETE')
