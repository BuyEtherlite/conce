...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4>✏️ Edit Payment Voucher #{{ $voucher->voucher_number }}</h4>
        <a href="{{ route('finance.vouchers.show', $voucher->id) }}" class="btn btn-secondary">
            <i class="fas fa-arrow-left me-1"></i>Back to Voucher
        </a>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('finance.vouchers.update', $voucher->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <small class="text-muted">Voucher number cannot be changed</small>
                        </div>
                    </div>
                            ...
                            <select class="form-select @error('supplier_id') is-invalid @enderror" id="supplier_id" name="supplier_id" required>
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" 
                                            {{ (old('supplier_id', $voucher->supplier_id) == $supplier->id) ? 'selected' : '' }}>
                            ...
                            <select class="form-select @error('account_id') is-invalid @enderror" id="account_id" name="account_id" required>
                                ...
                                @foreach($accounts as $account)
                                    <option value="{{ $account->id }}" 
                                            {{ (old('account_id', $voucher->account_id) == $account->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" 
                                   id="amount" name="amount" value="{{ old('amount', $voucher->amount) }}" required>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('reference_number') is-invalid @enderror" 
                                   id="reference_number" name="reference_number" value="{{ old('reference_number', $voucher->reference_number) }}">
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('finance.vouchers.show', $voucher->id) }}" class="btn btn-secondary me-2">Cancel</a>
